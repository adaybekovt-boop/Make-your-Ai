...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {dialog === 'economy' && <Modal title="Как идут дела" onClose={() => setDialog(null)}>
      <div className="economy-summary"><div><span>Регулярная выручка в час</span><strong className="text-green">+{money(economy.revenuePerHour)}</strong></div><div><span>Токены и подписки</span><strong>+{money(economy.serverRevenuePerHour)}</strong></div><div><span>Аренда башен — доход</span><strong>+{money(economy.propertyRevenuePerHour)}</strong></div><div><span>Электричество · {money(ELECTRICITY_PRICE_PER_KWH)}/кВт·ч</span><strong>−{money(economy.electricityPerHour)}</strong></div><div><span>Обслуживание серверов</span><strong>−{money(economy.maintenancePerHour)}</strong></div><div><span>Аренда помещений</span><strong>−{money(economy.rentPerHour)}</strong></div><div><span>Эксплуатация башен</span><strong>−{money(economy.propertyExpensesPerHour)}</strong></div><div><span>Зарплаты</span><strong>−{money(economy.salariesPerHour)}</strong></div><div className="summary-total"><span>Текущая прибыль в час</span><strong className={economy.profitPerHour < 0 ? 'text-warm' : 'text-green'}>{signedMoney(economy.profitPerHour)}</strong></div></div><p className="card-note">Суточные и разовые события, включая лицензии и контракты, проводятся отдельно. Установка сервера расширяет ёмкость аудитории, но не создаёт мгновенную выручку.</p><h3>За всё время</h3><div className="economy-summary"><div><span>Стартовый капитал</span><strong>{money(STARTING_CASH)}</strong></div><div><span>Получено выручки</span><strong>{money(state.game.totalRevenue)}</strong></div><div><span>Операционные расходы</span><strong>{money(state.game.totalExpenses)}</strong></div><div><span>Покупки минус продажи</span><strong>{money(state.game.totalCapex)}</strong></div></div>
    </Modal>}
