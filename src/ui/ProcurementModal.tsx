...
export function ProcurementModal() {
  ...
  return <Modal title={server ? 'Апгрейд чипа' : 'Закупка и монтаж'} onClose={actions.closeProcurement}>
    <div className="procurement-flow">
      <p className="card-description">Заказ → доставка на склад → монтаж{position ? ` · ячейка ${position.row + 1}, ${position.col + 1}` : ''}</p>
      {existingChassis ? <p>{CHASSIS[existingChassis].name} уже установлена. Оплачивается только чип. {server && 'После апгрейда прежний исправный чип вернётся на склад.'}</p> : <>
        <label>Что заказать<select aria-label="Состав заказа" value={kind} onChange={(event) => setKind(event.target.value as typeof kind)}><option value="kit">Шасси + чип</option><option value="chassis">Только шасси</option><option value="chip">Только чип на склад</option></select></label>
        <label>Шасси<select aria-label="Шасси" value={chassis} onChange={(event) => setChassis(event.target.value as ChassisId)}>{Object.values(CHASSIS).map((item) => <option key={item.id} value={item.id}>{item.name} · база {money(item.price)}</option>)}</select></label>
      </>}
      ...
      {mode !== 'chassis' && <label>Чип<select aria-label="Чип" value={selectedChip ?? ''} onChange={(event) => setChip(event.target.value as ChipId)}>{!choices.length && <option value="">Нет совместимых улучшений</option>}{choices.map((id) => <option key={id} value={id}>{CHIPS[id].name} · база {money(CHIPS[id].price)} · {CHIPS[id].powerKw} кВт</option>)}</select></label>}
